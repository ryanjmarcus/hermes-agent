...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. Hermes returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. When background execution is requested but the session cannot receive a detached result (and has no API-server session wake target), Hermes rejects admission without running the work inline. A full background pool or scheduling failure also returns `status: rejected`, `started: false`, and `queued: false`. No automatic retry has been queued: the existing job owner must retain the request and retry admission when its dependency changes or use another supported execution route. Explicitly requested synchronous delegation remains available to direct Python callers; a top-level model cannot select it through an unadvertised `background: false` argument.

Finite CLI turns (`chat --oneshot`, quiet queries, and noninteractive single queries) reject background delegation because the CLI process will exit after its answer. They do not return a worker handle for work that will be interrupted on exit. Run delegated work through an interactive CLI, gateway, or a session-continuable API-server owner instead. This check runs inside the model execution thread, so both quiet and threaded CLI paths enforce the same lifetime contract.
