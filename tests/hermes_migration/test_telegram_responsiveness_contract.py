...
def event(text="Please check this", message_id="401"):
    return MessageEvent(
        text=text, message_type=MessageType.TEXT, source=lane(), message_id=message_id
    )
...
def busy_runner(value, monkeypatch):
    ...
    worker = MagicMock()
    worker.get_activity_summary.return_value = {}
...
async def test_busy_worker_does_not_hold_receipt_and_topic_is_preserved(monkeypatch):
    ...
    async def send(**kwargs):
        sent.append(kwargs)
        return SendResult(success=True, message_id="502")
...
@pytest.mark.xfail(
    strict=True,
    reason="Native busy receipts are suppressed for 30 seconds; every request receipt is not guaranteed",
)
async def test_each_distinct_busy_request_gets_a_receipt(monkeypatch):
    value = adapter()
    value._send_with_retry = AsyncMock(
        return_value=SendResult(success=True, message_id="502")
    )
    runner, _ = busy_runner(value, monkeypatch)
    for number in ("401", "402"):
        await runner._handle_active_session_busy_message(
            event(message_id=number), build_session_key(lane())
        )
...
@pytest.mark.xfail(
    strict=True,
    reason="Cold native dispatch starts processing without an application-level receipt",
)
async def test_cold_request_has_ack_before_worker_dispatch():
    value = adapter()
    order = []
    value._message_handler = AsyncMock()
...
def callback(choice_id):
    query = SimpleNamespace(
        ...
        from_user=SimpleNamespace(id=777, first_name="Tester"),
        answer=AsyncMock(),
...
async def test_native_choice_resolves_directly_while_worker_is_busy():
    ...
    try:
        ...
        assert (
            ...
            in update.callback_query.answer.call_args.kwargs["text"]
        )
    finally:
        with clarify_gateway._lock:
            clarify_gateway._entries.pop(cid, None)
            clarify_gateway._session_index.pop(key, None)


@pytest.mark.asyncio
@pytest.mark.xfail(
    strict=True,
    reason="Native clarify routing state is adapter-local and does not survive restart",
)
async def test_choice_can_route_after_adapter_restart():
    ...
    try:
        ...
        second = adapter()
        second._is_callback_user_authorized = lambda *args, **kwargs: True
...
@pytest.mark.xfail(
    strict=True,
    reason="Legacy busy_text_mode=queue bypasses native busy acknowledgment",
)
async def test_queue_text_mode_still_acknowledges(monkeypatch):
    value = adapter()
    value._send_with_retry = AsyncMock(
        return_value=SendResult(success=True, message_id="503")
    )
    runner, _ = busy_runner(value, monkeypatch)
...
async def test_cold_processing_starts_native_typing_while_handler_waits():
    ...
    value.send_typing = send_typing
    value.stop_typing = AsyncMock()
    value._message_handler = handler
    task = asyncio.create_task(
        value._process_message_background(event(), build_session_key(lane()))
    )
    try:
...
@pytest.mark.asyncio
async def test_native_steer_accepts_followup_without_interrupt_queue_or_canned_ack(
    monkeypatch,
):
    import gateway.run as module
    ...
    monkeypatch.setattr(
        ...
        lambda: {"display": {"busy_steer_ack_enabled": False}},
    )
    assert await runner._handle_active_session_busy_message(
        event(), build_session_key(lane())
    )
    worker.steer.assert_called_once_with("Please check this")
    worker.interrupt.assert_not_called()
    assert not value._pending_messages
    value._send_with_retry.assert_not_awaited()
